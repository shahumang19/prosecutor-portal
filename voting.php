	...
	<div id="voting">
		<!--  candidates view  -->
		 <?php
			...
			$stmt1= $conTbl->query("select distinct position_id from vote where cid in (select cid from candidate where eid = (select eid from election where status=0)) and pid = (select pid from prosecutor where email = '".$_SESSION['username']."')");
			$stmt1->execute();
			$temp = $stmt1->fetchAll();
			$voted=array();
			$cnt = 0;
			foreach($temp as $vt)
			{
				...
				// 	//	echo "<script> alert('$re[name]');</script>";
					...
					<?php
						...
						if($check==false)
						{
							...
							$candidate[$i]=$data[5];
							$i++;
				?>	<div style=" background-color:white; border-radius:10px;overflow:auto;margin-bottom:20px;">
				<center>
				<label for="position" style=" text-align:center;text-transform:uppercase;"><h2><b><?php echo $re['name']; ?> </b></h2></label>
						</center>
				<?php
						}
					?>
				...
				<!-- individual candidates view -->
				 ...
				 <div class="card_view-header">
					<h4 style="text-transform:capitalize;"><b><?php echo $data[0] . " " .$data[1]?> </b></h4>
				</div>
				<div class="card_view-body">
					<img src="<?php echo $data[2]?> " style="float:left; width:100px;heigth:150px;margin-right:5px;"></img>
					<p> <?php echo $data[3]?> </p>
				</div>
				
				<div class="card_view-footer">
					<?php
					if(in_array($data[5],$voted))
					{
						if($v == 'voted')
						{
					...
					{
						echo "<button type='submit' name='voter' width='100%'> $v </button>";
					}
					?>
					</div>
				</div>
